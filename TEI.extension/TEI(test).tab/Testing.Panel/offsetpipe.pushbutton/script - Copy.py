...
def run():
    ...
    with revit.Transaction("Pipe Offset Split"):
        ...
        seg_a   = Pipe.Create(doc, system_type_id, pipe_type_id, level_id, pipe_start, pt_A)
        seg_b   = Pipe.Create(doc, system_type_id, pipe_type_id, level_id, pt_O1,  pt_new_end)
        seg_off = Pipe.Create(doc, system_type_id, pipe_type_id, level_id, pt_A,   pt_O1)
        
        # Set diameter while still inside the transaction
        if diameter_ft:
            set_diameter(seg_a,   diameter_ft)
            set_diameter(seg_b,   diameter_ft)
            set_diameter(seg_off, diameter_ft)

        # Connect elbows
        c_a_end   = get_connector_at(seg_a,   pt_A,  tol=0.05)
        c_off_st  = get_connector_at(seg_off, pt_A, tol=0.05)
        if c_a_end and c_off_st:
            doc.Create.NewElbowFitting(c_a_end, c_off_st)

        c_off_end = get_connector_at(seg_off, pt_O1, tol=0.05)
        c_b_start = get_connector_at(seg_b,   pt_O1,  tol=0.05)
        if c_off_end and c_b_start:
            doc.Create.NewElbowFitting(c_off_end, c_b_start)
